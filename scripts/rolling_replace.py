...
config on new instances launched from that point on. So an orchestration must then
occur (using something like terraform local-exec)

1. Set the instance to draining (see: https://docs.aws.amazon.com/AmazonECS/latest/developerguide/container-instance-draining.html)
2. Wait for it to drain (I'd recommend lowering the AWS default deregistration delay of 300s)
3. Terminate the instance
4. Wait for the new instance (with the new AMI) to launch and tasks to be scheduled there.
...
2. For simplicity, it follows a break one, make one pattern but does so in user defined
batch size (default is 3 batches) to ensure only a small loss in capacity.
...
script. e.g. at least, use an ASG minimum of 2 instances and deploying in 2 batches
to ensure that 1 instance is always available.
...
def parse_args():
    ...
    parser.add_argument('--batches', required=False, default=3,
            help='Number of batches in replacing instances. Default 3, use carefully.')
...
def get_services(ecs_client, cluster_name):
    services = []
    service_arns = ecs_client.list_services(cluster=cluster_name).get('serviceArns')
...
def rolling_replace_instances(ecs, ec2, cluster_name, batches, ami_id, force):
    ...
    utils.printInfo(f'You have {len(instances)} instances. Terminating in batches of {batch_count}') 
    ...
    for to_drain in instance_batches:
        ...
        ecs.update_container_instances_state(cluster=cluster_name, status='DRAINING', containerInstances=to_drain)
        utils.printInfo('wait for drain to complete...')
        ...
        while len(done_instances) < len(to_drain):
            ...
            for container_instance in response.get('containerInstances'):
                ...
                if running_tasks > 0:
                    print_progress()
                    continue
                if not instance_id in done_instances:
    ...
    utils.printSuccess('EC2 instance replacement complete!')
